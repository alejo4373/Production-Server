...
const updateTodoValidators = withValidationErrorHandler([
  oneOf([
    body("completed_at")
      .isISO8601()
      .withMessage("completed_at must be a ISO8601 date string"),
    body("completed")
      .isBoolean()
      .withMessage("completed must be true or false"),
    body("due_at")
      .isISO8601()
      .withMessage("due_at must be a ISO8601 date string"),
    body("text").trim().notEmpty().withMessage("cannot be empty"),
    body("value")
      .isInt({ min: 1 })
      .withMessage("must be a valid positive number"),
    body("list_id").isInt({ min: 1 }).withMessage("list "),
  ]),
]);
...
const retrieveTodosValidators = withValidationErrorHandler([
  query("completed_at")
    .optional()
    .isISO8601()
    .withMessage("must be in the format YYYY-DD-MM"),
  query("updated_at")
    .optional()
    .isISO8601()
    .withMessage("must be in the format YYYY-DD-MM"),
  ...
  query("client_tz")
    .optional()
    .custom((client_tz, { req }) => utils.isValidTimeZone(client_tz)),
  query("completed")
    .optional()
    .isBoolean()
