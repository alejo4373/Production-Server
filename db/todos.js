...
const createTodo = async (todo) => {
  try {
    const SQL = `
      INSERT INTO todos(owner_id, text, value ${
        todo.list_id ? ", list_id" : ""
      })
        VALUES($/owner_id/, $/text/, $/value/ ${
          todo.list_id ? ", $/list_id/" : ""
        })
      RETURNING *
    `;
...
const removeTodo = async (id, owner_id) => {
  ...
  try {
    todo = await db.one(
      `DELETE FROM todos WHERE id = $/id/ AND owner_id = $/owner_id/ 
      RETURNING *`,
      { id, owner_id }
    );
    return todo;
...
const getTodo = async (todoId, ownerId) => {
  const SQL =
    "SELECT * FROM todos WHERE id = $/todoId/ AND owner_id = $/ownerId/";
  try {
